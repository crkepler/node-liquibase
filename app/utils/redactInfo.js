  ...
  traverse(obj).forEach(function redactor(value) {
    if (isSensitiveKey(this.key)) {
      ...
      if (typeof value == "string") {
        let redactedText1 = value.replace(
          /--password="(.*?)"/gim,
          "--password=[REDACTED]"
        );
        let redactedText2 = redactedText1.replace(
          /--referencePassword="(.*?)"/gim,
          "--referencePassword=[REDACTED]"
        );
        //With escaped quotes:
        let redactedText3 = redactedText2.replace(
          /--password=\\"(.*?)\\"/gim,
          "--password=[REDACTED]"
        );
        let redactedText4 = redactedText3.replace(
          /--referencePassword=\\"(.*?)\\"/gim,
          "--referencePassword=[REDACTED]"
        );
